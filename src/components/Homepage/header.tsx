...
export default function Header({ onNavigate }: HeaderProps = {}) {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-black/65 backdrop-blur-xl border-b border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.35)]">
      <nav className="max-w-7xl mx-auto px-6 py-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <a
            href="/auth/login"
            className="hidden md:inline-flex items-center justify-center rounded-full border border-pink-500/30 bg-pink-600/10 px-4 py-2 text-sm font-semibold text-pink-300 transition duration-200 hover:bg-pink-600/20 hover:text-white"
          >
            Login
          </a>
          <a
