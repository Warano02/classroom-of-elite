...
interface ISystem {
    sys: "favorites" | "wl" | 'ip' | "complete"
}
export function SystemCollection({ sys }: ISystem) {
        ...
        <div className="flex-1 w-full overflow-auto">
            <div className="p-4 md:p-6 space-y-6">
                <div className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <h3 className="text-lg font-medium mb-1">No {sys == "favorites" ? "favorite" : sys == "ip" ? "Progress" : sys == "wl" ? "Watch later" : "Complete"} yet</h3>
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl border bg-card">
                    ...
                    <div>
                        <h2 className="text-lg font-semibold">
                            {sys == "favorites" ? "Favorite Course" : sys == "ip" ? "In Progress Course" : sys == "wl" ? "Watch Later Course" : "Completed Course"}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                            {favoriteBookmarks.length} course
                            {favoriteBookmarks.length !== 1 ? "s" : ""} marked as {sys == "favorites" ? "favorite" : sys == "ip" ? "in progress" : sys == "wl" ? "watch later" : "completed"}
                        </p>
                    </div>
                </div>
